...
function sourceQuoteInstruction(sourceUsage: SourceUsageSettings) {
  ...
  return "- For solving help or method teaching: Do not only point the student to pages. When a student asks to pull up, read, or quote a specific passage from selected uploaded class material, quote the relevant passage exactly with source/page context, then explain or paraphrase it. For problem-statement lookup, give only the problem text in the Problem section; do not include location/source context, offers, hints, or commentary in that section. Preserve visible line breaks when available; if the extracted problem text is flattened, add best-effort markdown line breaks only around clear structure such as headings, problem numbers, and enumerated parts. Do not refuse on generic copyright grounds for selected class materials, and do not invent missing words.";
}
